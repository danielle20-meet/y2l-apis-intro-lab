...
def movies():
    json_string = """
                    ...
                    """
    newpy=json.loads(json_string)
    return render_template('movie.html', movie=newpy)
...
@app.route('/dogs')
def dog_breeds():
    respone=requests.get("https://dog.ceo/api/breeds/list/all")
    pyd=json.loads(respone.content)
    listdog=pyd["message"]
    return render_template('dogs.html',dogl=listdog)
